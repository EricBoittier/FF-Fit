...
required.add_argument(
    "-o", "--output", type=str, help="Name of your output", required=True
)
optional = parser.add_argument_group("optional arguments")
optional.add_argument(
    "-l", "--linear", type=int, default=0, help="Use if your molecule is linear"
)
optional.add_argument(
    "-ns",
    "--number_of_samples",
...
optional.add_argument(
    "-T", "--temperature", type=float, default=30000, help="Temperature of the sampling"
)
...
def R(natom, fct, T=300, linear=False):
    ...
    # random_num = [i * 1000 for i in range(size)]
...
def write_xyz(
    eq_coord, mass, vib_disp, fcts, elem_num, natom, out_name, T=300, linear=False
):
    coord = eq_coord + new_coord(natom, vib_disp, mass, fcts, T=T, linear=linear)

    elem_name = [t.element[i] for i in elem_num]
    comment = "Molecule generated with normal mode sampling"
