...
def corrfunc(x, y, **kws):
    (r, p) = pearsonr(x, y)
    ax = plt.gca()
    ax.annotate("r = {:.6f} ".format(r), xy=(0.1, 0.9), xycoords=ax.transAxes)

    MAE = np.mean(np.abs(x - y))
    ax.annotate("MAE = {:.6f}".format(MAE), xy=(0.1, 0.8), xycoords=ax.transAxes)
    RMSE = np.sqrt(np.mean((x - y) ** 2))
    ax.annotate("RMSE = {:.6f}".format(RMSE), xy=(0.1, 0.7), xycoords=ax.transAxes)
