...
from ff_energy import calculate_cross_val_uncertainty, calculate_std_uncertainty
...
class UncertaintyQuantifier:
    ...
    def calculate_uncertainty(self, split=0.8, keys=None):
        ...
        self.data["FIT_scaled"] = self.scaler.fit_transform(
            self.data[self.key].values.reshape(-1, 1)
        ).T[0]

        self.data["TARGET_scaled"] = self.scaler.transform(
            self.data["TARGET"].values.reshape(-1, 1)
        ).T[0]

        self.data["SE_scaled"] = self.data["FIT_scaled"].std() / np.sqrt(
            self.data["FIT_scaled"].shape[0]
        )
        ...
        train_idx = np.random.choice(
            self.data.index, size=int(len(self.data) * split), replace=False
        )
        ...
        if "bayesian_uncertainty" in keys:
            ...
            bayesian_uncertainty = calculate_bayesian_uncertainty(
